...
{
    /// <summary>
    /// Class that allows to create single socket that can be connected only with one client and send a large count of bytes in fastest ways
    /// </summary>
    public class StreamServer
    {
        ...
        /// <summary>
        /// IPAddress that is allowed to be connected to the server
        /// </summary>
        ...
        /// Constructor with custon server settings
        /// </summary>
        public StreamServer(int port, IPAddress iPAddress)
        {
            Encoding = Encoding.Unicode;
            Port = port;
            Socket = new Socket(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp);
        ...
        /// Constructor with custon server settings
        /// </summary>
        public StreamServer(int port, IPAddress iPAddress, Encoding encoding, int bufferSize)
        {
            Encoding = encoding;
            Port = port;
            Socket = new Socket(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp);
        ...
        public void Start()
        {
            try
            {
                ...
                Client = new ServerClient(Port, newSocket);
            }
            catch (Exception)
            {
                Started = false;
